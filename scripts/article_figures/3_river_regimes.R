...
metab_q_avg_metrics <- sp_data %>%
  ...
  mutate(
    discharge_category = case_when(
      Discharge_joined_median <= 0.1 ~ "[0 - 0.1]",
      Discharge_joined_median > 0.1 & Discharge_joined_median <= 1 ~
        "(0.1 - 0.5]",
      ...
      as_factor() %>%
      fct_reorder(Discharge_joined_median)
  )
...
metab_q_avg_metrics %>%
  ...
  geom_label(
    data = . %>% filter(siteID %in% c("KC6", "SBM", "BRW", "BEC")),
    aes(label = siteID),
    nudge_x = .3,
    label.size = NA,
    alpha = 0
...
metab_q_avg_metrics %>%
  filter(siteID %in% c("KC6", "SBM", "BRW", "BEC")) %>%
  select(
    siteID,
    GPP_mean,
    ER_mean,
    Discharge_joined_mean,
    Discharge_joined_skewness
  )
...
metrics_plot <-
  ...
  mutate(
    ...
    text.site = c(
      ...
      "Brewery Creek (WI, USA)",
      "Krycklan C6 (Sweden)",
      "Saddleback mountain (NH, USA)"
    )
  )
...
sp_data %>%
  ...
  geom_text(
    data = metrics_plot,
    aes(x = 610, y = 190, label = text.site),
    size = 3,
    color = "black",
    hjust = 1
  ) +
  stat_density_2d_filled(
    aes(alpha = after_stat(level), fill = site_type),
    linewidth = .1,
    geom = "polygon",
    contour = TRUE,
  ...
  scale_fill_manual(
    values = c("cadetblue4", "darkolivegreen4", "coral4", "goldenrod3")
  ) +
  scale_color_manual(
    values = c("cadetblue4", "darkolivegreen4", "coral4", "goldenrod3")
  ) +
