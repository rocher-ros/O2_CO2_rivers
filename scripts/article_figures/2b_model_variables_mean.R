...
met_with_chem %>% 
  drop_na(alk_mmol_m3) %>% 
  summarise(across(c(alk_mmol_m3), list(min = min, max= max, median=median, sd= sd,  count = ~ n())))

gw_usgs %>% 
  drop_na(co2_umol_l) %>% 
  summarise(across(c(co2_umol_l), list(min = min, max= max, median=median, sd= sd, count = ~ n())))

gw_usgs %>% 
  drop_na(oxygen_umol_l) %>% 
  summarise(across(c(oxygen_umol_l), list(min = min, max= max, median=median, sd= sd, count = ~ n())))
...
plot_er <- ellipses_er + inset_element(density_er, left = 0.33, bottom = 0.5, right = .99, top= .99)
...
pars_K <- 
  tibble(bigK = K_qs$k600.md/avg_values$depth,
         ER.day = avg_values$ER/32*1e+3, 
         GPP.day = avg_values$GPP/32*1e+3, 
         q.m3s = avg_values$discharge,
         depth.m= avg_values$depth,
...
ellipses_K <- sim_out_K %>% 
  filter(date== as.Date("2020-07-07")) %>% 
  ggplot(aes(x = co2.mod-co2.air, y = o2.mod-o2.air, color = K600.d*depth.m, fill= K600.d*depth.m, group= K600.d*depth.m))+
  ellipse_plotting_theme +
...
density_k <- 
  ...
  density_plotting_theme +
  geom_segment(data= K_qs, aes(x=k600.md/avg_values$depth, xend= k600.md/avg_values$depth, y=1, yend= 1.35),  color= "gray30", linetype=1)+
  geom_text(data=K_qs, aes(x=k600.md/avg_values$depth, y= 1.42, label=q ), angle = 90, size= 2.6)+
  scale_fill_scico(palette= "oslo", begin = 0.3, end=0.97)+
  scale_x_continuous(limits=c(quantile(site_metab$k600.md, 0.05),
                              quantile(site_metab$k600.md, 0.97)), expand = c(0,0),
                     name= expression(m~d^-1))+
  scale_y_continuous( expand = c(0,0), name="", limits= c(1,1.5))+
  guides(x = guide_axis(minor.ticks = TRUE))+
  theme(axis.title.x = element_text( hjust = 1.12, vjust = -1.7,   color= "gray20", size= 10),
        plot.margin = margin(-12,10,-10,-10))

plot_k <- ellipses_K + inset_element(density_k, left = 0.33, bottom = 0.5, right = .99, top= .99)
...
density_alk <- met_with_chem %>% 
  ...
  guides(x = guide_axis(minor.ticks = TRUE))+
  theme(axis.title.x = element_text( hjust = 1.1, vjust = -1.6,  color= "gray20", size= 10),
        plot.margin = margin(-12,10,-12,-10))


plot_alk <- ellipses_alk + inset_element(density_alk, left = 0.33, bottom = 0.5, right = .99, top= .99)
...
density_pq <- 
  ...
  density_plotting_theme+
  geom_segment(data= pq_qs, aes(x=pq, xend= pq, y=1, yend= 4),  color= "gray30", linetype=1)+
  geom_text(data=pq_qs, aes(x=pq, y= 5, label=q ), angle = 90, size= 2.6, color= "gray10")+
  ...
  scale_x_continuous(limits=c(quantile(quotients_distr$pq, 0.05, na.rm= TRUE),
                     ...
                     name="", expand = c(0,0), breaks= c(0.7, 1,1.3, 1.6), minor_breaks = seq(0.7, 1.6, by = .1))+
  scale_y_continuous( expand = c(0,0), name="", limits= c(1,6))+
  guides(x = guide_axis(minor.ticks = TRUE))+
  theme(plot.margin = margin(-12,10,-10,-10))

plot_pq <- ellipses_pq + inset_element(density_pq, left = 0.33, bottom = 0.5, right = .99, top= .99)
...
density_rq <- 
  ...
  scale_x_continuous(limits=c(quantile(quotients_distr$rq, 0.05, na.rm= TRUE),
                     ...
                     breaks= c(0.6, 1,1.4, 1.8), minor_breaks = seq(0.6, 1.8, by = .2))+
  scale_y_continuous( expand = c(0,0), name="", limits= c(1,4))+
  guides(x = guide_axis(minor.ticks = TRUE))+
  theme(plot.margin = margin(-12,10,-10,-10))

plot_rq <- ellipses_rq + inset_element(density_rq, left = 0.33, bottom = 0.5, right = .99, top= .99)
...
density_gw <- 
  ...
  density_plotting_theme+
  geom_segment(data= gw_qs, aes(x=gw_frac*100, xend= gw_frac*100, y=1, yend= 1.93), color= "gray30", linetype=1)+
...
plot_gw <- ellipses_gw + inset_element(density_gw, left = 0.33, bottom = 0.5, right = .99, top= .99)
...
density_gw_co2 <- 
  ...
  density_plotting_theme+
  geom_segment(data= gw_co2_qs, aes(x=gw_co2, xend= gw_co2, y=1, yend= 1.0017), color= "gray30", linetype=1)+ #from umol/L to ppm *1000*0.04477565
  ...
  guides(x = guide_axis(minor.ticks = TRUE))+
  theme(axis.title.x = element_text( hjust = 1.14, vjust = -1.05,  color= "gray20", size= 10),
        plot.margin = margin(-11,10,-15,-10))

plot_gw_co2 <- ellipses_gw_co2 + inset_element(density_gw_co2, left = 0.33, bottom = 0.5, right = .99, top= .99) 
...
density_gw_o2 <- 
  ...
  density_plotting_theme+
  geom_segment(data= gw_o2_qs, aes(x=gw_o2, xend= gw_o2, y=1, yend= 1.0074), color= "gray30", linetype=1)+
  geom_text(data=gw_o2_qs, aes(x=gw_o2, y= 1.0089, label=q ), angle = 90, size= 2.6)+
  ...
  scale_x_continuous(limits=c(quantile(gw_usgs$oxygen_mg_l/32*1e+3, 0.05, na.rm= TRUE),
                     ...
                     name=expression(mu*mol~L^-1), expand = c(0.008,0), #breaks=c(2,6,10),
                     minor_breaks = scales::breaks_width(50))+
  scale_y_continuous( expand = c(0,0), name="", limits= c(1,1.01))+
  guides(x = guide_axis(minor.ticks = TRUE))+
  theme(axis.title.x = element_text( hjust = 1.1, vjust = -1,  color= "gray20", size= 10),
        plot.margin = margin(-11,10,-15,-10))

plot_gw_o2 <- ellipses_gw_o2 + inset_element(density_gw_o2, left = 0.33, bottom = 0.5, right = .99, top= .99) 
...
o2_example + co2_example + pH_example +
  plot_layout(ncol = 1)
