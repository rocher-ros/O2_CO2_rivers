...
median(site_metab$gw_frac, na.rm = TRUE)
mean(site_metab$gw_frac, na.rm = TRUE)
...
quotients_distr <- tibble(
  pq = rnorm(10000, mean = 1.18, sd = .31), #from literature values (SM) #1.18, sd= 0.31
  rq = rnorm(10000, mean = 1.2, sd = 0.45)
) # from Berggren et al. 2012
...
met_with_chem %>%
  summarise(across(
    c(GPP, ER, K600),
    list(min = min, max = max, median = median, sd = sd, count = ~ n())
  ))
...
avg_values <- site_metab %>%
  summarise(across(c(ER, GPP, K600, k600.md, discharge, depth, gw_frac), \(x) {
    median(x, na.rm = TRUE)
  })) %>%
  mutate(
    PQ = median(quotients_distr$pq),
...
ellipse_plotting_theme <- list(
  ...
  scale_y_continuous(limits = c(-103, 110)),
  theme_classic(),
  theme(
...
density_plotting_theme <- list(
  geom_density_ridges_gradient(linewidth = 0),
  theme_classic(),
  theme(
    legend.position = "none",
...
ER_qs <- site_metab %>%
  reframe(ER = quantile(ER, q_selected, na.rm = TRUE), q = q_selected)

#run the model with all fixed except ER
pars_ER <-
  tibble(
    ER.day = ER_qs$ER / 32 * 1e+3,
...
ellipses_er <- sim_out_ER %>%
  ...
  ggplot(aes(
    x = co2.mod - co2.air,
    y = o2.mod - o2.air,
    color = ER.day * 32 / 1e+3,
    fill = ER.day * 32 / 1e+3,
    group = ER.day * 32 / 1e+3
  )) +
...
density_er <-
  ...
  geom_segment(
    data = ER_qs,
    aes(x = ER, xend = ER, y = 1, yend = 1.4),
    color = "gray30",
    linetype = 1
  ) +
  geom_text(
    data = ER_qs,
    aes(x = ER, y = 1.6, label = q),
    angle = 90,
    size = 2.6
  ) +
  scale_fill_scico(
    name = expression(g ~ O[2] ~ m^3 ~ L^-1),
    palette = "lajolla",
    begin = 0.1,
    end = 0.8
  ) +
  ...
  scale_x_continuous(
    limits = c(quantile(site_metab$ER, 0.05), quantile(site_metab$ER, 0.95)),
    expand = c(0, 0),
  ...
  theme(
    ...
    plot.margin = margin(-12, 5, -15, -10)
  )


plot_er <- ellipses_er +
  inset_element(density_er, left = 0.33, bottom = 0.5, right = .99, top = .99)

plot_er
## GPP plot ----
GPP_qs <- site_metab %>%
  reframe(GPP = quantile(GPP, q_selected), q = q_selected)

#run the model with all fixed except GPP
pars_GPP <- tibble(
  GPP.day = GPP_qs$GPP / 32 * 1e+3,
...
ellipses_gpp <- sim_out_GPP %>%
  ...
  ggplot(aes(
    x = co2.mod - co2.air,
    y = o2.mod - o2.air,
    color = GPP.day * 32 / 1e+3,
    fill = GPP.day * 32 / 1e+3,
    group = GPP.day * 32 / 1e+3
  )) +
...
density_gpp <- ggplot(site_metab, aes(x = GPP, y = 1, fill = after_stat(x))) +
  ...
  geom_segment(
    data = GPP_qs,
    aes(x = GPP, xend = GPP, y = 1, yend = 1.7),
    color = "gray30",
    linetype = 1
  ) +
  geom_text(
    data = GPP_qs,
    aes(x = GPP + c(-0.02, 0.03, 0.03, 0, 0), y = 1.9, label = q),
    angle = 90,
    size = 2.6
  ) +
  scale_fill_scico(palette = "bamako", direction = -1, begin = 0.2, end = 0.9) +
  scale_x_continuous(
    limits = c(quantile(site_metab$GPP, 0.05), quantile(site_metab$GPP, 0.95)),
    expand = c(0, 0.1),
    name = expression(g ~ O[2] ~ m^-2 ~ d^-1),
    breaks = c(1, 3, 5, 7)
  ) +
...
# put both together
plot_gpp <- ellipses_gpp +
  inset_element(density_gpp, left = 0.33, bottom = 0.5, right = .99, top = .99)


## K plot ----
K_qs <- site_metab %>%
  reframe(
    K600 = quantile(K600, q_selected),
    k600.md = quantile(k600.md, q_selected),
    q = q_selected
  )
...
pars_K <-
  tibble(
    bigK = K_qs$k600.md / avg_values$depth,
    ER.day = avg_values$ER / 32 * 1e+3,
    GPP.day = avg_values$GPP / 32 * 1e+3,
    q.m3s = avg_values$discharge,
    depth.m = avg_values$depth,
...
ellipses_K <- sim_out_K %>%
  ...
  ggplot(aes(
    x = co2.mod - co2.air,
    y = o2.mod - o2.air,
    color = K600.d * depth.m,
    fill = K600.d * depth.m,
    group = K600.d * depth.m
  )) +
  ellipse_plotting_theme +
...
density_k <-
  ggplot(site_metab, aes(x = k600.md, y = 1, fill = after_stat(x))) +
  density_plotting_theme +
  geom_segment(
    data = K_qs,
    aes(
      x = k600.md / avg_values$depth,
      xend = k600.md / avg_values$depth,
      y = 1,
      yend = 1.35
    ),
    color = "gray30",
  ...
  geom_text(
    data = K_qs,
    aes(x = k600.md / avg_values$depth, y = 1.42, label = q),
    angle = 90,
    size = 2.6
  ) +
  ...
  scale_x_continuous(
    limits = c(
      quantile(site_metab$k600.md, 0.05),
      quantile(site_metab$k600.md, 0.97)
    ),
    expand = c(0, 0),
    name = expression(m ~ d^-1)
  ) +
  scale_y_continuous(expand = c(0, 0), name = "", limits = c(1, 1.5)) +
  guides(x = guide_axis(minor.ticks = TRUE)) +
  theme(
    axis.title.x = element_text(
      hjust = 1.12,
...
plot_k <- ellipses_K +
  inset_element(density_k, left = 0.33, bottom = 0.5, right = .99, top = .99)
...
alk_qs <- met_with_chem %>%
  reframe(
    alk_mmol_m3 = quantile(alk_mmol_m3, q_selected, na.rm = TRUE),
    q = q_selected
  )
...
ellipses_alk <- sim_out_alk %>%
  ...
  labs(x = "", y = "", title = expression("Alkalinity"))
...
density_alk <- met_with_chem %>%
  ...
  geom_segment(
    data = alk_qs,
    aes(x = alk_mmol_m3, xend = alk_mmol_m3, y = 1, yend = 1.00057),
    color = "gray30",
    linetype = 1
  ) +
  geom_text(
    data = alk_qs,
    aes(x = alk_mmol_m3, y = 1.00072, label = q),
    angle = 90,
    size = 2.6
  ) +
  ...
  scale_x_continuous(
    limits = c(
      quantile(met_with_chem$alk_mmol_m3, 0.05, na.rm = TRUE),
      quantile(met_with_chem$alk_mmol_m3, 0.95, na.rm = TRUE)
    ),
    expand = c(0, 0),
    breaks = c(500, 2500, 4500),
    name = expression(mu * mol ~ L^-1),
    minor_breaks = scales::breaks_width(500)
  ) +
...
plot_alk <- ellipses_alk +
  inset_element(density_alk, left = 0.33, bottom = 0.5, right = .99, top = .99)

## Plot PQ ----

pq_qs <- quotients_distr %>%
  reframe(
    pq = quantile(pq, c(0.1, 0.3, 0.5, 0.7, 0.9), na.rm = TRUE),
    q = q_selected
  )
...
density_pq <-
  ...
  geom_segment(
    data = pq_qs,
    aes(x = pq, xend = pq, y = 1, yend = 4),
    color = "gray30",
    linetype = 1
  ) +
  geom_text(
    data = pq_qs,
    aes(x = pq, y = 5, label = q),
    angle = 90,
    size = 2.6,
    color = "gray10"
  ) +
  ...
  scale_x_continuous(
    limits = c(
      quantile(quotients_distr$pq, 0.05, na.rm = TRUE),
      quantile(quotients_distr$pq, 0.95, na.rm = TRUE)
    ),
    name = "",
    expand = c(0, 0),
    breaks = c(0.7, 1, 1.3, 1.6),
    minor_breaks = seq(0.7, 1.6, by = .1)
  ) +
  scale_y_continuous(expand = c(0, 0), name = "", limits = c(1, 6)) +
  guides(x = guide_axis(minor.ticks = TRUE)) +
  theme(plot.margin = margin(-12, 10, -10, -10))

plot_pq <- ellipses_pq +
  inset_element(density_pq, left = 0.33, bottom = 0.5, right = .99, top = .99)


## Plot RQ ----

rq_qs <- quotients_distr %>%
  reframe(rq = quantile(rq, q_selected, na.rm = TRUE), q = q_selected)
...
density_rq <-
  ...
  geom_segment(
    data = rq_qs,
    aes(x = rq, xend = rq, y = 1, yend = 2.9),
    color = "gray30",
    linetype = 1
  ) +
  geom_text(
    data = rq_qs,
    aes(x = rq, y = 3.4, label = q),
    angle = 90,
    size = 2.6
  ) +
  ...
  scale_x_continuous(
    limits = c(
      quantile(quotients_distr$rq, 0.05, na.rm = TRUE),
      quantile(quotients_distr$rq, 0.95, na.rm = TRUE)
    ),
    name = "",
    expand = c(0, 0),
    breaks = c(0.6, 1, 1.4, 1.8),
    minor_breaks = seq(0.6, 1.8, by = .2)
  ) +
...
plot_rq <- ellipses_rq +
  inset_element(density_rq, left = 0.33, bottom = 0.5, right = .99, top = .99)

## Plot %GW ----

gw_qs <- site_metab %>%
  reframe(
    gw_frac = quantile(gw_frac, q_selected, na.rm = TRUE) / 100,
    q = q_selected
  )
...
ellipses_gw <-
  ...
  ggplot(aes(
    x = co2.mod - co2.air,
    y = o2.mod - o2.air,
    color = log(gw.frac * 100),
    fill = log(gw.frac * 100),
    group = log(gw.frac * 100)
  )) +
...
density_gw <-
  ...
  geom_segment(
    data = gw_qs,
    aes(x = gw_frac * 100, xend = gw_frac * 100, y = 1, yend = 1.93),
    color = "gray30",
    linetype = 1
  ) +
  geom_text(
    data = gw_qs,
    aes(x = gw_frac * 100, y = 2.13, label = q),
    angle = 90,
    size = 2.6
  ) +
  scale_fill_scico(
  ...
  scale_x_log10(
    limits = c(
      quantile(site_metab$gw_frac, 0.05, na.rm = TRUE),
      quantile(site_metab$gw_frac, 0.95, na.rm = TRUE)
    ),
    name = "%",
    expand = c(0, 0.07),
    breaks = c(0.3, 1, 3, 10),
...
plot_gw <- ellipses_gw +
  inset_element(density_gw, left = 0.33, bottom = 0.5, right = .99, top = .99)
...
gw_co2_qs <- gw_usgs %>%
  reframe(
    gw_co2 = quantile(co2_umol_l, q_selected, na.rm = TRUE),
    q = q_selected
  )
...
ellipses_gw_co2 <-
  ...
  labs(
    y = "",
    x = expression(CO[2] ~ departure ~ (mu * mol ~ L^-1)),
    title = expression(CO[2] ~ "in groundwater")
  )
...
density_gw_co2 <-
  ...
  geom_segment(
    data = gw_co2_qs,
    aes(x = gw_co2, xend = gw_co2, y = 1, yend = 1.0017),
    color = "gray30",
    linetype = 1
  ) + #from umol/L to ppm *1000*0.04477565
  geom_text(
    data = gw_co2_qs,
    aes(x = gw_co2, y = 1.0021, label = q),
    angle = 90,
    size = 2.6
  ) +
  ...
  scale_x_continuous(
    limits = c(
      quantile(gw_usgs$co2_umol_l, 0.05, na.rm = TRUE),
      quantile(gw_usgs$co2_umol_l, 0.95, na.rm = TRUE)
    ),
    breaks = c(250, 1000, 1750),
    name = expression(mu * mol ~ L^-1),
    expand = c(0, 0.07)
  ) +
  ...
  theme(
    ...
    plot.margin = margin(-11, 10, -15, -10)
  )

plot_gw_co2 <- ellipses_gw_co2 +
  inset_element(
    density_gw_co2,
    left = 0.33,
    bottom = 0.5,
...
gw_o2_qs <- gw_usgs %>%
  ...
  reframe(
    gw_o2 = quantile(oxygen_mg_l / 32 * 1e+3, q_selected, na.rm = TRUE),
    q = q_selected
  )
...
ellipses_gw_o2 <-
  ...
  labs(
    y = "",
    x = expression(CO[2] ~ departure ~ (mu * mol ~ L^-1)),
    title = expression(O[2] ~ "in groundwater")
  )

density_gw_o2 <-
  ggplot(
    gw_usgs,
    aes(x = oxygen_mg_l / 32 * 1e+3, y = 1, fill = after_stat(x))
  ) +
  density_plotting_theme +
  geom_segment(
    data = gw_o2_qs,
    aes(x = gw_o2, xend = gw_o2, y = 1, yend = 1.0074),
    color = "gray30",
    linetype = 1
  ) +
  geom_text(
    data = gw_o2_qs,
    aes(x = gw_o2, y = 1.0089, label = q),
    angle = 90,
    size = 2.6
  ) +
  ...
  scale_x_continuous(
    limits = c(
      quantile(gw_usgs$oxygen_mg_l / 32 * 1e+3, 0.05, na.rm = TRUE),
      quantile(gw_usgs$oxygen_mg_l / 32 * 1e+3, 0.95, na.rm = TRUE)
    ),
    name = expression(mu * mol ~ L^-1),
    expand = c(0.008, 0), #breaks=c(2,6,10),
...
ggsave(
  "plots/main/fig2_main_drivers.png",
  plots_all,
  height = 8,
  width = 8,
  scale = 1.22
...
pars_example <-
  tibble(
    ...
    alk.mmolm3 = avg_values$alk_mmol_m3,
    gw.frac = avg_values$gw_frac / 100,
    gw.O2conc = avg_values$gw.O2conc,
    gw.CO2conc = avg_values$gw.CO2conc
  )

## run the simulation with the set parameters

dat_example <- co2_o2_sim(pars_example) %>%
...
plot_temp <-
  ...
  geom_text(
    aes(
      x = c(
        as.POSIXct("2020-07-14 20:00:00"),
        as.POSIXct("2020-07-14 20:00:00")
      ),
      y = c(15.5, 16.4),
      label = c("Air", "Water")
    ),
    color = c("lightblue", "blue2"),
    fontface = "bold",
    hjust = 0
  ) +
  scale_x_datetime(breaks = "12 hours", date_labels = "%H") +
...
pH_example <- ggplot() +
  geom_point(data = dat_example, aes(date.time, pH)) +
  scale_x_datetime(breaks = "12 hours", date_labels = "%H") +
  theme_classic() +
  labs(x = "Hour", y = "pH")
