...
files_lakes <- list.files("empirical data/lake data/vachon2020",pattern="*.csv", full.names = T)
...
co2_density_plot <- rivers_lakes %>%
  ggplot()+
  geom_vline(xintercept = 0, color= "gray40")+
  geom_density(aes(CO2dep, y= after_stat(scaled), fill= type, color= type), alpha=.3)+
  geom_point(data=. %>% summarise(CO2dep = median(CO2dep), .by= type), 
             aes(x= CO2dep, y=0.2, color=type), shape= 19, size= 3, show.legend = FALSE)+
  scale_fill_manual(values= c("dodgerblue3", "darkorange"))+
  scale_color_manual(values= c("dodgerblue3", "darkorange"))+
...
o2_density_plot <- rivers_lakes %>%
  ggplot()+
  geom_vline(xintercept = 0, color= "gray40")+
  geom_density(aes(O2dep, y= after_stat(scaled), fill= type, color= type), alpha=.3)+
  geom_point(data=. %>% summarise(O2dep = median(O2dep), .by= type), 
             aes(x= O2dep, y=0.2, color=type), shape= 19, size= 3, show.legend = FALSE)+
  scale_y_continuous(expand = c(0,0))+
  scale_fill_manual(values= c("dodgerblue3", "darkorange"))+
  scale_color_manual(values= c("dodgerblue3", "darkorange"))+
