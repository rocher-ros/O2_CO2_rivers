...
elipse <- function(CO2, O2) {
  if (length(CO2) > 48) {
    ...
    out <- tibble(
      width = ell.len[2], #output is the length and width of the ellipse
      length = ell.len[1]
    )
  } else {
...
regression_metrics <- function(CO2, O2) {
  if (length(CO2) > 48) {
    ...
    out <- tibble(
      EQ = 1 / abs(slope), #the output is the EQ (inverse of slope)
      corr = corr
    ) #and the pearson r
  } else {
...
catchment_lengths <- read_csv(
  "prepared data/river data/USGS_data/catchment_lengths_properties.csv"
) %>%
...
site_metab <- daily_metab %>%
  ...
  mutate(
    groundwater_m3s = avg_discharge_m3s -
      (tot_length - DO.tdist80_median / 1000) / tot_length * avg_discharge_m3s,
    gw_frac = groundwater_m3s / avg_discharge_m3s * 100,
    spQ_mmday = discharge_mean * 3600 * 24 / catchment_area * 1000
  ) %>%
  mutate(
    gw_frac = if_else(gw_frac > 100, NA, gw_frac),
    gw_frac = if_else(gw_frac < 0.1, NA, gw_frac)
  )


#export file for analysis in other scripts
...
site_metab %>%
  filter(
    tot_area / (catchment_area / 1e+6) > .5,
    tot_area / (catchment_area / 1e+6) < 5,
    gw_frac <= 100,
    gw_frac > .01,
    discharge_median > 0.07,
    discharge_mean < 1e+4
  ) %>%
  ...
  labs(
    ...
    y = "% discharge from groundwater within the 80% footprint reach",
...
data_for_model <- site_metab %>%
  filter(
    ...
    ER_mean > -20,
    discharge_mean < 500,
    discharge_mean > 0.006,
    tot_area / (catchment_area / 1e+6) > .5,
    tot_area / (catchment_area / 1e+6) < 5,
    gw_frac <= 100
  ) %>%
  mutate(NEP_mean = ER_mean + GPP_mean, k_md = K600_mean * depth_mean) %>%
...
RCC_smoothed <- tibble(
  ...
  mutate(
    k600.md.lowerBound = ifelse(
      k600.md.lowerBound <= 0,
      0.001,
      k600.md.lowerBound
    )
  )
...
plot_nep <- ggplot(RCC_smoothed) +
  geom_ribbon(
    aes(discharge, ymin = NEP.lowerBound, ymax = NEP.upperBound),
    alpha = .3,
    fill = "yellow4"
  ) +
...
plot_gpp <- ggplot(RCC_smoothed) +
  geom_ribbon(
    aes(discharge, ymin = GPP.lowerBound, ymax = GPP.upperBound),
    alpha = .3,
    fill = "chartreuse3"
  ) +
...
plot_er <- ggplot(RCC_smoothed) +
  geom_ribbon(
    aes(discharge, ymin = ER.lowerBound, ymax = ER.upperBound),
    alpha = .3,
    fill = "red4"
  ) +
...
plot_gw <-
  ...
  ggplot() +
  geom_ribbon(
    aes(discharge, ymin = gw.lowerBound, ymax = gw.upperBound),
    alpha = .3,
    fill = "dodgerblue2"
  ) +
...
plot_k_md <-
  ...
  ggplot() +
  geom_ribbon(
    aes(discharge, ymin = k600.md.lowerBound, ymax = k600.md.upperBound),
    alpha = .3,
    fill = "gray40"
  ) +
...
params_rcc <- tibble(
  discharge_mean = 10^(seq(
    from = log10(min(data_for_model$discharge_mean)),
    to = log10(max(data_for_model$discharge_mean)),
    length.out = 50
  ))
) %>%
  mutate(
    GPP.day = predict(lw_GPP, newdata = .) / 32 * 1e+3,
...
plot_ellipse_rcc <- sim_out %>%
  mutate(
    q.categories = case_when(
      q.m3s < 0.1 ~ "very small",
...
ellipse_metrics <- sim_out %>%
  ...
  mutate(CO2dep_mmolm3 = co2.mod - co2.air, O2dep_mmolm3 = o2.mod - o2.air) %>%
  summarise(
