...
co2_o2_sim <- function(params) {
  ...
  for (i in 2:nrow(df)) {
    ...
    df$o2.mod[i] = df$o2.mod[i - 1] + #O2 from previous time step
      (df$q.gw[i] * gw.O2conc) / (df$area.m2[i] * df$depth.m[i]) * dt -
      (df$q.gw[i] * df$o2.mod[i - 1]) / (df$area.m2[i] * df$depth.m[i]) * dt + #groundwater inputs
    ...
    df$dic.mod[i] = df$dic.mod[i - 1] + #dic from previous time step
      (df$q.gw[i] * gw.DICconc) / (df$area.m2[i] * df$depth.m[i]) * dt -
      (df$q.gw[i] * df$dic.mod[i - 1]) / (df$area.m2[i] * df$depth.m[i]) * dt - #groundwater inputs from Hall & Tank 2005
